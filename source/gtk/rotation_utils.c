...
void rotate_pixbuf(GdkPixbuf *src_pixbuf, GdkPixbuf *dst_pixbuf, double angle)
{
  ...
  printf("📏 Source Width: %d, Source Height: %d, Destination Width: %d, Destination Height: %d, Channels: %d\n",
  ...
  for (int y = 0; y < dst_height; y++)
  {
    ...
    {
      int src_x = (int)((x - dst_width / 2) * cos_angle - (y - dst_height / 2) * sin_angle + src_width / 2);
      int src_y = (int)((x - dst_width / 2) * sin_angle + (y - dst_height / 2) * cos_angle + src_height / 2);

      if (src_x >= 0 && src_x < src_width && src_y >= 0 && src_y < src_height)
      {
        for (int c = 0; c < n_channels; c++)
        {
          guchar *src_pixel = gdk_pixbuf_get_pixels(src_pixbuf) + src_y * gdk_pixbuf_get_rowstride(src_pixbuf) + src_x * n_channels + c;
          guchar *dst_pixel = gdk_pixbuf_get_pixels(dst_pixbuf) + y * gdk_pixbuf_get_rowstride(dst_pixbuf) + x * n_channels + c;
          *dst_pixel = *src_pixel;
        }
      }
    }
