...
GtkWidget *init_menu_bar(GtkWidget *image_widget)
{
  ...
  // Creates processing menu => menu bar
  processing_menu = gtk_menu_new();
  processing_menu_item = gtk_menu_item_new_with_label("🌑 Processing");
  gtk_menu_item_set_submenu(GTK_MENU_ITEM(processing_menu_item), processing_menu);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu_bar), processing_menu_item);

  // Creates grayscale menu item => processing_menu
  ...
  gtk_menu_shell_append(GTK_MENU_SHELL(processing_menu), contrast_item);

  // Creates median filter menu item => processing_menu
  ...
  thresholding_menu_item = gtk_menu_item_new_with_label("📈 Thresholding");
  ...
  // Creates adaptive thresholding menu item => thresholding_menu
  adaptive_item = gtk_menu_item_new_with_label("Adaptive (local) thresholding");
  ...
  // Creates Otsu thresholding menu item => thresholding_menu
  otsu_item = gtk_menu_item_new_with_label("Otsu (global) thresholding");
  ...
  // Creates Sauvola thresholding menu item => thresholding_menu
  sauvola_item = gtk_menu_item_new_with_label("Sauvola");
  g_signal_connect(sauvola_item, "activate", G_CALLBACK(on_sauvola_thresholding_clicked), image_widget);
  gtk_menu_shell_append(GTK_MENU_SHELL(thresholding_menu), sauvola_item);

  // Creates rotation menu => menu bar
  rotation_menu = gtk_menu_new();
  rotation_menu_item = gtk_menu_item_new_with_label("↪️ Rotation");
  ...
  g_signal_connect(auto_rotate_item, "activate", G_CALLBACK(on_auto_rotate_clicked), image_widget);
  gtk_menu_shell_append(GTK_MENU_SHELL(rotation_menu), auto_rotate_item);

  // Creates documentation menu item => menu bar
  documentation_menu_item = gtk_menu_item_new_with_label("✴️ Documentation");
  g_signal_connect(documentation_menu_item, "activate", G_CALLBACK(on_documentation_clicked), image_widget);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu_bar), documentation_menu_item);
...
static void activate(GtkApplication *app, gpointer user_data)
{
  ...
  const char *button_labels[] = {"Rotate left", "Rotate right"};
       ...
       i < sizeof(button_labels) / sizeof(button_labels[0]); i++)
  {
    ...
    if (strcmp(button_labels[i], "Rotate left") == 0)
    {
