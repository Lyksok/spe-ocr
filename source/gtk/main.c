...
gboolean on_window_configure_event(GtkWidget *widget, GdkEvent *event,
  ...
  gtk_window_set_default_size(GTK_WINDOW(widget), screen_width / 2,
                              screen_height);
  return FALSE;
}
...
int main(int argc, char *argv[]) {
  ...
  G_CALLBACK(on_angle_entry_activate), &left_angle);   // updates when the user
  presses enter g_signal_connect(right_angle_entry, "activate",
  ...
  g_signal_connect(run_button, "clicked", G_CALLBACK(on_run_clicked),
                   image_widget);

  // // Connect GtkEntry signals
  // GtkWidget *left_angle_entry = GTK_WIDGET(gtk_builder_get_object(builder,
  // "left_angle_entry")); g_signal_connect(left_angle_entry, "activate",
  // G_CALLBACK(on_left_angle_entry_activate), NULL);

  // GtkWidget *right_angle_entry = GTK_WIDGET(gtk_builder_get_object(builder,
  // "right_angle_entry")); g_signal_connect(right_angle_entry, "activate",
  // G_CALLBACK(on_right_angle_entry_activate), NULL);

  // // Set expand property to FALSE for the buttons
  // gtk_widget_set_hexpand(run_button, FALSE);
  // gtk_widget_set_vexpand(run_button, FALSE);
  GtkWidget *left_angle_entry =
      GTK_WIDGET(gtk_builder_get_object(builder, "left_angle_entry"));
  GtkWidget *right_angle_entry =
      GTK_WIDGET(gtk_builder_get_object(builder, "right_angle_entry"));
  ...
  g_signal_connect(left_angle_entry, "activate",
                   G_CALLBACK(on_left_angle_entry_activate),
                   &left_angle); // updates when the user presses enter
  g_signal_connect(
      right_angle_entry, "activate", G_CALLBACK(on_right_angle_entry_activate),
      &right_angle); // updates when the user presses enterpresses enter
  // Connect the buttons to their handlers / callbacks
  ...
  gtk_css_provider_load_from_path(css_provider, "source/gtk/style/style.css",
                                  NULL);
