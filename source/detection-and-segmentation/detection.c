...
BoundingBox *closest_bounding_boxes(BoundingBox *box, BoundingBox **boxes,
                                    int len, Point *src, Point *dest, int l) {
...
void get_all_links(BoundingBox **boxes, int len, Point **src, Point **dest) {
  for (int i = 0; i < len; i++) {
    BoundingBox *closest =
        closest_bounding_boxes(boxes[i], boxes, len, *src, *dest, i);
...
int main(int argc, char** argv)
{
    ...
    while (1)
    {
        ...
        for (int i=0; i<len; i++)
        {
            ...
            {
                int x1 =
(int)((double)boxes[i]->p1.x/(double)surface->w*(double)width); int y1 =
(int)((double)boxes[i]->p1.y/(double)surface->h*(double)height); int x2 =
(int)((double)boxes[i]->p2.x/(double)surface->w*(double)width); int y2 =
(int)((double)boxes[i]->p2.y/(double)surface->h*(double)height);
                //int rect[]={boxes[i]->p1.x, boxes[i]->p1.y,
boxes[i]->p2.x-boxes[i]->p1.x, boxes[i]->p2.y-boxes[i]->p1.y}; int
rect[]={x1,y1,x2-x1,y2-y1}; draw_rect(renderer, rect);
            }
        }
//        for(int i=0; i<min_len; i++)
//        {
//            src[i] =
(Point){(int)((double)src[i].x/(double)surface->w*(double)width),
//                (int)((double)src[i].y/(double)surface->h*(double)height)};
//        }
//        for(int i=0; i<min_len; i++)
//        {
//            dest[i] =
(Point){(int)((double)dest[i].x/(double)surface->w*(double)width),
