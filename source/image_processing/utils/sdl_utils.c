...
void draw_rect2(SDL_Surface *surface, SDL_Renderer *renderer, BoundingBox *box,
  ...
  SDL_Rect r;
  int x1 = max(0, -p + (int)ceil(((double)box->p1.x / (double)surface->w *
                                  (double)width)));
  int y1 = max(0, -p + (int)ceil(((double)box->p1.y / (double)surface->h *
                                  (double)height)));
  int x2 = min(width - 1, p + (int)ceil(((double)box->p2.x /
                                         (double)surface->w * (double)width)));
  int y2 = min(
      height - 1,
      p + (int)ceil(((double)box->p2.y / (double)surface->h * (double)height)));
  r.x = x1;
  r.y = y1;
  r.w = x2 - x1;
  r.h = y2 - y1;
