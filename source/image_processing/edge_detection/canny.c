    ...
    for (int x = 1; x < width - 1; x++)
    {
      ...
      Uint8 mag =
          (*magnitude)[y * surface->w +
                       x]; // Get the gradient magnitude for the current pixel
      double dir = ((*direction)[y * surface->w + x] * 2.0 * PI / 255.0) -
                   PI; // Get the gradient direction for the current pixel and
                       // convert it to radi

      /** By initializing neighbour1 and neighbour2 to 255, we ensure that if
       * the gradient direction does not match any of the specified ranges, the
       * current pixel will be suppressed because its magnitude will not be
      ...
      else if (5 * PI / 8 <= dir && dir < 7 * PI / 8)
      {
        ...
        neighbour2 = (*magnitude)[(y + 1) * surface->w + x +
                                  1]; // Get the pixel value to the bottom-right
      }
