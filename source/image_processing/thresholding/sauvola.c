...
void compute_sauvola_args(int width, int height, int **integral,
                          int **integral_sq, int x, int y, double *mean,
                          double *stddev) {
  // w = win_size
  ...
  double sum = (*integral)[y2 * width + x2] + (*integral)[y1 * width + x1] -
               (*integral)[y1 * width + x2] - (*integral)[y2 * width + x1];

  double sum_sq =
      (*integral_sq)[y2 * width + x2] + (*integral_sq)[y1 * width + x1] -
      (*integral_sq)[y1 * width + x2] - (*integral_sq)[y2 * width + x1];
...
void sauvola_thresholding(SDL_Surface *surface, struct parameters *param) {
  ...
  for (int j = 0; j < surface->h; j++) {
    for (int i = 0; i < surface->w; i++) {
      ...
      integral[j * width + i] =
          value + (i > 0 ? integral[j * width + i - 1] : 0) +
          (j > 0 ? integral[(j - 1) * width + i] : 0) -
          (i > 0 && j > 0 ? integral[(j - 1) * width + i - 1] : 0);

      integral_sq[j * width + i] =
          value * value + (i > 0 ? integral_sq[j * width + i - 1] : 0) +
  ...
  for (int j = 0; j < surface->h; j++) {
    for (int i = 0; i < surface->w; i++) {
      double mean;
      double stddev;
      compute_sauvola_args(width, height, &integral, &integral_sq, i, j, &mean,
                           &stddev);
